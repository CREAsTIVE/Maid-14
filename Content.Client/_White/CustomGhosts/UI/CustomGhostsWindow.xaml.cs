...
public sealed partial class CustomGhostsWindow : DefaultWindow
{
    ...
    private ProtoId<CustomGhostPrototype>? _pendingGhostProtoId;
    private TimeSpan _nextSendTime;
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        if (_timing.RealTime >= _nextSendTime)
            SendPendingGhost();
    }
    ...
    private void OnPressed(BaseButton.ButtonEventArgs args)
    {
        ...
        _pendingGhostProtoId = protoId;

        if (_timing.RealTime >= _nextSendTime)
            SendPendingGhost();
    }

    private void SendPendingGhost()
    {
        if (_pendingGhostProtoId is not { } protoId)
            return;

        _pendingGhostProtoId = null;
        _nextSendTime = _timing.RealTime + SelectionCooldown;
